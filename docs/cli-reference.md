...
Each host also reports `databaseBinding.path` (the proposed resolved absolute database), `databaseBinding.previousPath` (the existing managed binding, or `null` for a generic/new installation), and a scope notice. The interactive final confirmation shows the same proposed and previous paths. Confirmed apply writes explicit `--db` instructions into each selected Skill and records the binding in its local managed manifest; changing the selected database rebinds those Skills. Repeating the same setup is idempotent. No global config or environment is written, and bare CLI commands do not discover this binding. Agents must apply the installed argument guidance to every CRM command. Direct Skill upgrades preserve an existing binding; fresh direct installs remain generic. Uninstall/reinstall through the direct installer returns to generic guidance without deleting CRM data.
...
Known setup destinations are Pi at `~/.agents/skills`, Claude Code at `$CLAUDE_CONFIG_DIR/skills` or `~/.claude/skills`, and Hermes at `$HERMES_HOME/skills` when `HERMES_HOME` is nonempty, otherwise `~/.hermes/skills`. `HERMES_HOME` identifies the active profile root (for example, `~/.hermes/profiles/work`); setup detects and installs into that profile only. Explicit `integration install-skill` and `integration uninstall-skill --destination` paths remain authoritative; their default remains `~/.agents/skills`. Pi and Claude Code require fresh sessions; Hermes needs a fresh session or gateway restart. Setup only reports this guidance and never restarts a process.
