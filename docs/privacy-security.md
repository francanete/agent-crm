...
The setup catalog uses `~/.agents/skills` for Pi, `$CLAUDE_CONFIG_DIR/skills` or `~/.claude/skills` for Claude Code, and `$HERMES_HOME/skills` (or `~/.hermes/skills` when unset or empty) for Hermes. Setup never edits `PATH`, shell profiles, systemd/launchd services, or gateway configuration, and never restarts a host. A background gateway may have a different `PATH` than the invoking terminal; its administrator must diagnose and correct that environment separately.
